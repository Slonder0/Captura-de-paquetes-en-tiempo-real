            ...
            if(dato[13] == 0x00){
               ...
               if(tipo == 17){
                ...
                if(origen == 53 or destino == 53){
                    ...
                    printf("\n\t  DNS\n");
                    printf("-----------------------\n");
                    ID_DNS(dato,archivo,tipo);
                    Flags_DNS(dato,archivo,tipo);
                    QD = QDcount(dato,archivo,tipo);
                    AN = ANcount(dato,archivo,tipo);
                    NS = NScount(dato,archivo,tipo);
                    AR = ARcount(dato,archivo,tipo);
                    ///Question
                    for(int i=0; i<QD; i++){
                        NombreQD_DNS(dato,archivo,tipo);
                        Tipo_DNS(dato,archivo,tipo);
                        Class_DNS(dato,archivo,tipo);
                    }
                    ///Answer
                    for(int i=0; i<AN; i++){
                        NombreAN_DNS(dato,archivo,AN,tipo);
                        Tipo_DNS(dato,archivo,tipo);
                        Class_DNS(dato,archivo,tipo);
                        TTL_DNS(dato,archivo,tipo);
                        tipo = RDATA(dato,archivo,tipo);
                    }
